...
<form action="<?php echo esc_url( home_url( '/' ) ); ?>" method="get" class="search-form" role="search">
	<div>
		...
		<button class="search-submit" type="submit"><?php echo sahar_get_svg( array( 'icon' => 'search' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?><span class="screen-reader-text"> <?php echo esc_html_x( 'Search', 'submit button', 'sahar' ); ?></span></button>
	</div>
</form>
